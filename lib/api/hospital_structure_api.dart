...
class HospitalStructureApi {
  static String get _base => Urlconfig.serverUrl;

  // 병동 조회
  // /api/hospital/structure/part?hospital_code=1
  static Future<List<Map<String, dynamic>>> fetchWards({required int hospitalCode}) async {
    final uri = Uri.parse('$_base/api/hospital/structure/part?hospital_code=$hospitalCode');
  ...
  static Future<List<Map<String, dynamic>>> fetchFloors({required int hospitalStCode}) async {
    final uri = Uri.parse('$_base/api/hospital/structure/floor?hospital_st_code=$hospitalStCode');
    final decoded = await HttpHelper.getJson(uri);
